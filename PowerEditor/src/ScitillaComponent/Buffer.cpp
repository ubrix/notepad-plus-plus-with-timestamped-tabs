...
BufferID FileManager::newEmptyDocument()
{
	...
	//TCHAR nb[10];
	TCHAR nb1[21]; //Ubrix new row
	TCHAR nb2[10]; //Ubrix new row

	//wsprintf(nb, TEXT("%d"), nextUntitledNewNumber());
	//newTitle += nb;
	_timeNow = time(NULL); //Ubrix new row
	wcsftime(nb1, 21, TEXT("%Y-%m-%d %H%M%S"), localtime(&_timeNow)); //Ubrix new row
	wsprintf(nb2, TEXT(" %d"), nextUntitledNewNumber()); //Ubrix new row
	newTitle = nb1; //Ubrix new row
	newTitle += nb2; //Ubrix new row
